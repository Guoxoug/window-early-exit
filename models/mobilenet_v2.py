...
class MobileNetV2(nn.Module):
    ...
    def _forward_impl(
        self, x: Tensor,
        return_features=False, feature_threshold=torch.inf
    ) -> Tensor:
        ...
        x = self.avgpool(x)
        x = self.flatten(x).clamp(max=feature_threshold)
        # this was left out by accident for the training of prev models
        # x = self.drop(x) 
        if not self.q_last_layer and self.q:
            x = self.dequant(x)
        feats = x
        if self.q:
            feats = self.dequant2(feats)
        
        # note that older models weren't trained with dropout 
        if self.dropout:
            x = self.drop(x)
        x = self.fc(x)
